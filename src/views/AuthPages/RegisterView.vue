...
async function handleSubmit() {
	...
	try {
		const response = await ofetch<RegisterAPI>(
			"/api/auth/register",
			{
				method: "POST",
				body: {
...
<template>
	<div class="flex w-full h-screen overflow-hidden">
		...
		<WelcomeComponent />
		<div class="w-full md:w-1/2 bg-gray-50 dark:bg-[#1c2735ff] flex items-center justify-center">
			<div class="w-full max-w-sm mx-4 md:mx-0">
				<h1 class="cursor-pointer text-4xl font-medium mb-8 text-center bg-gradient-to-r from-blue-700 to-blue-400 bg-clip-text text-transparent">
					注册
				</h1>
